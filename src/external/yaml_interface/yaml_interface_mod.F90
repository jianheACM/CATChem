...
!! \brief High-level Fortran interface for yaml-cpp
!! \author CATChem Development Team
!! \date 2025
!!
!! This module provides a high-level Fortran interface to yaml-cpp
!! with generic procedures that can handle all data types seamlessly.
!!
module yaml_interface_mod
   ...
   !> Opaque handle for YAML node
   type :: yaml_node_t
      type(c_ptr) :: ptr = c_null_ptr
   end type yaml_node_t

   !> Generic interface for getting values from YAML
   !! This allows uniform syntax: call yaml_get(node, key, value, rc)
   !! for any supported data type
   interface yaml_get
   ...
   !! This allows uniform syntax: call yaml_set(node, key, value, rc)
   !! for any supported data type
   interface yaml_set
   ...
   !! This allows uniform syntax: call yaml_get_array(node, key, values, rc)
   !! for any supported array type
