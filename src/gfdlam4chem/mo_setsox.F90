...
CONTAINS
  ...
  subroutine setsox( press, plonl, dtime, tfld, qfld, &
    ...
    if ( .not. module_is_initialized ) then
       call errmsg ('setsox','setsox_init must be called first.', .true.)
    end if
...
!          call mpp_clock_end(isoropia_clock_id)
  ...
  !<f1p
  subroutine setsox_init(trop_option,tracer_names)
...
!       end if
!    else
!       call errmsg ('setsox','nh4no3 must be definedx.', .true.)
!    end if
...
!           end if
!        else
!           call errmsg ('setsox','ano3 needs to be defined', .true.)
!        end if
!     END IF
