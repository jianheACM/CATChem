...
!   </TEMPLATE>                                                                        
!-----------------------------------------------------------------------
      subroutine fastjx_init(me,master)
!-----------------------------------------------------------------------
...
!----------------------------------------------------------------------
      ...
      if (me==master) &
        write(*,*)'Read external fastjx data'
...
! Read in fast-J X-sections (spectral data)
!-->      call RD_XXX(JXUNIT,'FJX_spec.dat')
      call RD_XXX(me,master,'INPUT/FJX71_spec.dat')
...
!-->      call RD_PROF(JXUNIT,'atmos_std.dat')
      ...
      if (me==master) &
        write(*,*)'FASTJX: ', TITLEJXX

! Read in photolysis rates used in chemistry code and mapping onto FJX J's
!---CTM call:  read in J-values names and link to fast-JX names
!-->      call RD_JS_JX(9,'FJX_j2j.dat', TITLEJXX,NJXX)
      call RD_JS_JX(me,master,'INPUT/FJX71_j2j.dat', TITLEJXX,NJXX)

      call RD_SOLAR(me,master,'INPUT/PHOTON_18bin.txt')
...
!-----------------------------------------------------------------------
    ...
    subroutine fastjx_photo (me, master,                                &
...
!      write(*,*) 'SZA ',SZA
      if (SZA .gt. 98.d0) then
          ZPJ(:,:) = 0.0
!          if (me==master) then
!            write(*,*) 'It is dark condition at ', hr,minute,sec
!          endif
          goto 99
      endif
...
!<<<<<<<<<<<<<<<<<<<<<<<<fast-J initialization subroutines<<<<<<<<<<<<<<
      ...
      subroutine RD_XXX(me,master,NAMFIL)
!-----------------------------------------------------------------------
...
!                     MPP_SEQUENTIAL, MPP_MULTI, MPP_SINGLE)
      ...
      open (NUN,FILE=trim(NAMFIL),action='read',form='formatted', &
...
!-----------------------------------------------------------------------
      subroutine RD_JS_JX(me,master,NAMFIL,TITLEJX,NJX)
!-----------------------------------------------------------------------
...
!                     MPP_SEQUENTIAL, MPP_MULTI, MPP_SINGLE)
      NUNIT = 66
      open (NUNIT,FILE=trim(NAMFIL),action='read', form='formatted',&
                 status='old',iostat=ios)
...
!         write(*,'(a)') CLINE
 ...
 20   close (NUNIT) !call mpp_close(NUNIT)
...
!   endif
      ...
      subroutine RD_SOLAR(me,master,NAMFIL)
      integer, intent (in) :: me
      integer, intent (in) :: master
...
!      real*8  XTINCT, REFF,RHO 
      ...
      if (L .gt. NAA_AM3 ) then 
         ...
         call errmsg ('ATMOS: fastjx_init:OPTICAM3','aerosol index out-of-range.', .true.)
      endif 
...
!                     MPP_SEQUENTIAL, MPP_MULTI, MPP_SINGLE)
     ...
     NJ1 = 66
     open (NJ1,FILE=trim(NAMFIL),action='read', form='formatted',&
                 status='old',iostat=ios)
