...
!----------------------------------------------------------------------
      ...
      use gfdl_sat_vapor_pres_mod, only : compute_qs      
      use      mo_errmsg, only : errmsg       
      use mo_chem_utls_mod,  only : get_spc_ndx, get_rxt_ndx, get_tracer_ndx  
...
!   </IN>
  ...
  subroutine fprate_init(o3_column_top_in,tracer_names,me,master)
!----------------------------------------------------------------------
...
!jul--  
      ...
      o3_ndx = get_tracer_ndx(tracer_names,'o3')
      if(  o3_ndx <1 )then
        o3_ndx = get_tracer_ndx(tracer_names,'OX')      
      end if
      so4_ndx     = get_tracer_ndx(tracer_names,'so4')
      bc1_ndx     = get_tracer_ndx(tracer_names,'bc1')
      bc2_ndx     = get_tracer_ndx(tracer_names,'bc2')
      oc1_ndx     = get_tracer_ndx(tracer_names,'oc1')
      oc2_ndx     = get_tracer_ndx(tracer_names,'oc2')
      soa_ndx     = get_tracer_ndx(tracer_names,'SOA')
      ssa_ndx(1)    = get_tracer_ndx(tracer_names,'seas1')
      ssa_ndx(2)    = get_tracer_ndx(tracer_names,'seas2')
      ssa_ndx(3)    = get_tracer_ndx(tracer_names,'seas3')
      ssa_ndx(4)    = get_tracer_ndx(tracer_names,'seas4')
      ssa_ndx(5)    = get_tracer_ndx(tracer_names,'seas5')
      dust_ndx(1)   = get_tracer_ndx(tracer_names,'dust1')
      dust_ndx(2)   = get_tracer_ndx(tracer_names,'dust2') 
      dust_ndx(3)   = get_tracer_ndx(tracer_names,'dust3')
      dust_ndx(4)   = get_tracer_ndx(tracer_names,'dust4')
      dust_ndx(5)   = get_tracer_ndx(tracer_names,'dust5')
       
      !nqa = get_tracer_ndx(tracer_names,'cld_amt')
      ...
      if(  bc1_ndx <1 )  call errmsg ('ATMOS: fphoto','Failed to find bc1_ndx', .true.)
      if(  bc2_ndx <1 )  call errmsg ('ATMOS: fphoto','Failed to find bc2_ndx', .true.)
      if(  oc1_ndx <1 )  call errmsg ('ATMOS: fphoto','Failed to find oc1_ndx', .true.)
      if(  oc2_ndx <1 )  call errmsg ('ATMOS: fphoto','Failed to find oc2_ndx', .true.)
      if(  soa_ndx <1 )  call errmsg ('ATMOS: fphoto','Failed to find soa_ndx', .true.)
      if(  ssa_ndx(1) <1 )  call errmsg ('ATMOS: fphoto','Failed to find ssa1_ndx', .true.)
      if(  ssa_ndx(2) <1 )  call errmsg ('ATMOS: fphoto','Failed to find ssa2_ndx', .true.)
      if(  ssa_ndx(3) <1 )  call errmsg ('ATMOS: fphoto','Failed to find ssa3_ndx', .true.)
      if(  ssa_ndx(4) <1 )  call errmsg ('ATMOS: fphoto','Failed to find ssa4_ndx', .true.)
      if(  ssa_ndx(5) <1 )  call errmsg ('ATMOS: fphoto','Failed to find ssa5_ndx', .true.)
      if(  dust_ndx(1) <1 )  call errmsg ('ATMOS: fphoto','Failed to find dust1_ndx', .true.)
      if(  dust_ndx(2) <1 )  call errmsg ('ATMOS: fphoto','Failed to find dust2_ndx', .true.)
      if(  dust_ndx(3) <1 )  call errmsg ('ATMOS: fphoto','Failed to find dust3_ndx', .true.)
      if(  dust_ndx(4) <1 )  call errmsg ('ATMOS: fphoto','Failed to find dust4_ndx', .true.)
      if(  dust_ndx(5) <1 )  call errmsg ('ATMOS: fphoto','Failed to find dust5_ndx', .true.)
      
      
!      if(  nqa <1 )  call errmsg ('ATMOS: fphoto','Failed to find cld_amt_ndx', .true.)
      if(  nqi <1 )  call errmsg ('ATMOS: fphoto','Failed to find ice_wat_ndx', .true.)
      if(  nql <1 )  call errmsg ('ATMOS: fphoto','Failed to find liq_wat_ndx', .true.)
!      if(  nqq <1 )  call errmsg ('ATMOS: fphoto','Failed to find c_ndx', .true.)
...
!-------------check model initialization -------------------------------
      ...
      if (.not. module_is_initialized) &
            call errmsg ('ATMOS: fphoto','fprate_init must be called first.', .true.)
#ifdef USE_MPI
!----------------------------------------------------------------------
...
!      end if
 ...
 do j=lbound(v1,dim=2),ubound(v1,dim=2)  ; do i=lbound(v1,dim=1),ubound(v1,dim=1)
    ...
    elseif (v1(i,j)+v2(i,j) == 0) then
      call errmsg("Error in set_aerosol_mc: ", "v1+v2 = 0: can not divide by 0", .true.)     
    else
