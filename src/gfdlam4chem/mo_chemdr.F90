...
!-----------------------------------------------------------------------
      ...
      if ( .not. module_is_initialized ) then
         call errmsg ('chemdr','chemdr_init must be called first.', .true.)
      end if
...
!   call mpp_clock_begin(photo_clock_id)
         if (.not. trop_option%do_fastjx_photo) then
           call photo( reaction_rates(:,:,:phtcnt), pmid, pdel, tfld, zmid, &
            ...
            call fphoto( me, master, &
                         reaction_rates(:,:,:phtcnt), &
                         pmid, pdel, &
                         tfld, zmid, &
                         col_dens, &
                         coszen, albedo, &
...
!</SUBROUTINE>
!<f1p
      subroutine chemdr_init(trop_option, tracer_names)
