...
!   </IN>
 subroutine atmos_radon_init (me,master,tracer_names,nradon, mask)
...
!----- set initial value of radon ------------
       ...
       n = get_tracer_ndx(tracer_names,search_name)
       if (n>0) then
         nradon(nn)=n
         if (nradon(nn) > 0 .and. me == master) write (*,30) trim(search_name), nradon(nn)
       endif
