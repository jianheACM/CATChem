...
use catchem_constants, only : PI, DEG_TO_RAD, AVOGNO, PSTD_MKS, SECONDS_PER_DAY
use gfdl_time_utls_mod, only : time_type, get_date, days_in_month, days_in_year, &
...
subroutine strat_chem_utilities_init( Time_init,me,master,age_factor_in, dclydt_factor_in, &
...
! dummy arguments
   ...
   type(time_type),  intent(in)                 :: Time_init,ch4_entry_in
...
! local variables
   ...
   integer :: unit, nc, n, year, outunit, lbcunit
...
!-----------------------------------------------------------------------
   ...
   fin='INPUT/' // TRIM(cfc_lbc_filename)
   inquire (file=trim(fin), exist=file_exist)
   if ( file_exist ) then
     open (unit=lbcunit, file=fin, action='read', form='formatted',&
                 status='old', iostat=ios)
   else
     print *,' Requested cfc_lbc file ',trim(fin),' not found'
     print *,' ** Stopped in strat_chem_utilities_init ** '
     stop 1220
   endif      ! end if_file_exist_block

   do nc = 1,15
     read(lbcunit,'(6E13.6)') chlb_dummy(:,nc)
   end do
   read(lbcunit,'(6E13.6)') ozb_dummy
   read(lbcunit,'(6E13.6)') tropc
   close(lbcunit)

   !if(me==master) write(*,*) 'Read cfc_lbc: ',tropc
...
!JianHe: not sure if cfc_entry should be set like this
...
!++lwh
subroutine strat_chem_dcly_dt(me,master,Time, phalf, is, js, &
                       age, cly, bry, dfdage,dclydt, dbrydt)
!--lwh
...
real, dimension(:,:,:,:), intent(in)  :: dfdage
!--lwh
...
!if (me==master) then
!   call get_date(Time,iyear, imon, iday, ihour, imin, isec )
!   write (*, *) 'Input time in strat_chem_dcly_dt: ',iyear, imon, iday, ihour, imin, isec
!end if
...
!if (me==master) then
!   call get_date(cfc_base_Time,iyear, imon, iday, ihour, imin, isec )
!   write (*, *) 'cfc_base_Time in strat_chem_dcly_dt: ',iyear, imon, iday, ihour, imin, isec
!   call get_date(cfc_offset,iyear, imon, iday, ihour, imin, isec )
!   write (*, *) 'cfc_offset in strat_chem_dcly_dt: ',iyear, imon, iday, ihour, imin, isec
!   call get_date(tropc_Time(1),iyear, imon, iday, ihour, imin, isec )
!   write (*, *) 'tropc_Time(1) in strat_chem_dcly_dt: ',iyear, imon, iday, ihour, imin, isec
!end if
...
!     cfc(i,j,:) = tropc(it1,:)*(1-dt1) + tropc(it2,:)*dt1
      ...
      if(err_msg /= '') then
         call errmsg('strat_chem_dcly_dt', trim(err_msg) , .true.)
      endif
...
!      if (me==master .and. k==13) then
!        write (*, *) 'Diag strat_chem_dcly_dt', i,j,dclydt(i,j,k),dfdage(i,j,k,:)
!      endif
...
!JianHe: we currently do not have extinct from radiation scheme
!So we read from initial condition
!In the future, we may connect to the radiation calculation
...
do i=1,il
   ...
   if(err_msg /= '') then
      call errmsg('strat_chem_get_extra_h2o', trim(err_msg) , .true.)
   endif 
...
if ( file_exist ) then
   open (unit=flb, file=filename, action='read', form='formatted', &
                 status='old',iostat=ios)
...
!---------------------------------------------------------------------
   ...
   call errmsg ('strat_chem_extra_h2o_init', &
                    'Failed to find input file '//trim(filename), .true.)
end if
