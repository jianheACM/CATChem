      ...
      private
      public  :: gfdl_lnox_init, gfdl_lnox_hook
...
logical                       :: module_is_initialized = .false.
      ...
      subroutine gfdl_lnox_init( me, master, lght_no_prd_factor, &
...
!----------------------------------------------------------------------
      ...
      if(me==master) then
         write(*,*) 'gfdl_lnox_init: Lightning NO production scaling factor = ',factor
         if (normalize_lght_no_prd_area) then
            write(*,*) 'gfdl_lnox_init: Normalize lightning NO production by grid cell area'
	 else
            write(*,*) 'gfdl_lnox_init: Normalize lightning NO production by grid cell (not area)'
	 end if
      end if
...
!----------------------------------------------------------------------
      ...
      end subroutine gfdl_lnox_init


      subroutine gfdl_lnox_hook( me, master, cldtop, cldbot, oro, zm, zint, t, &
                           prod_no, prod_no_col, area, lat, is, js)
!----------------------------------------------------------------------
...
!----------------------------------------------------------------------
      ...
      real, intent(out) :: prod_no_col(:,:)          ! production of NOx column (molec cm^-2 s^-1)
...
!      if (.not. module_is_initialized) return
   ...
   if (.not. module_is_initialized)  &
      call errmsg ('gfdl_lnox','gfdl_lnox_init must be called first.', .true.)
...
!--------------------------------------------------------------------------------
               ...
               cldbind =  cldbot(i,j) 
               !if (me==master) then
               !  write(*,*),'hook input cloud top and bottom level: ', cldtind,cldbind 
               !endif
               do
...
!--------------------------------------------------------------------------------
      ...
      end subroutine gfdl_lnox_hook

      end module gfdl_lnox_mod
