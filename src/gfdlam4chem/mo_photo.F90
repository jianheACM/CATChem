...
!        ... Open error exit
!-----------------------A-----------------------------------------------
        msg = 'PRATE_INIT: Failure opening file ' // TRIM(lpath) // TRIM(filename)
        call ENDRUN(msg)
     end if
...
!----------------------------------------------------------------------
        OPEN( unit   = unit, &
           file   = TRIM(lpath) // TRIM(filename_solarmin), &
           action = 'read', &
           status = 'old', &
           form   = 'formatted', &
           recl   = 4500, &
...
!        ... Open error exit
!----------------------------------------------------------------------
        msg = 'PRATE_INIT: Failure opening file ' // TRIM(lpath) // TRIM(filename_solarmin)
        call ENDRUN(msg)
        end if
...
!-----------------------------------------------------------------------------
      ...
      call errmsg('MO_PHOTO_MOD: ', msg, .true.)
