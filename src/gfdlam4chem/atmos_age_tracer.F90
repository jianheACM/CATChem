...
!   </OUT>
 subroutine atmos_age_tracer (lon, lat, pwt, age, age_dt)
...
!</ERROR>
   if (.not. module_is_initialized)  &
      call errmsg ('atmos_age_tracer','atmos_age_tracer_init must be called first.', .true.)
...
!   </IN>
 subroutine atmos_age_tracer_init( tracer_names, nage)
...
!       write ( stdlog(), nml=atmos_age_tracer_nml )
      ...
      n = get_tracer_ndx(tracer_names,'age')  ! age tracer
