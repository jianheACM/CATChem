MODULE module_gocartracm_driver
    USE gocartracm_Parameters
    USE gocartracm_Precision
    USE gocartracm_UpdateRconstWRF
    USE gocartracm_Integrator
!wkppc constants (include file)
! A few constants 
...
! locally define pointers to photolysis rates
     ...
     INTEGER, PARAMETER, PRIVATE :: Pj_cl2 = 49 
     INTEGER, PARAMETER, PRIVATE :: Pj_hocl = 50 
     INTEGER, PARAMETER, PRIVATE :: Pj_fmcl = 51 

CONTAINS
 
 SUBROUTINE gocartracm_driver(   &
             chem, dtstepc, &
...
!initialization, individual parameters set below 
    ...
    DO i=its, ite
     ...
     CALL gocartracm_Update_Rconst(  &
              ...
              Pj_mekooh, Pj_tolooh, Pj_terpooh, Pj_cl2, Pj_hocl, &
              Pj_fmcl,                                           &
              C_M, FIX(indf_H2O), TEMP &
              )
     CALL gocartracm_INTEGRATE(TIME_START, TIME_END, &
    ...
    END DO
    END DO
 END SUBROUTINE gocartracm_driver
END MODULE module_gocartracm_driver
