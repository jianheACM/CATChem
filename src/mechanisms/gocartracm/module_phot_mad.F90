    ...
    USE catchem_config,     only: p_o3,p_qi,p_qc,p_qv
    USE catchem_constants,  only: pi=>con_pi,rd=>con_rd,g=>con_g
    use MODULE_CHEMVARS
    use phot_data_mod
      ...
      INTEGER, PARAMETER :: nabv = 1, nj = 200, nreakj = 23
...
!
    CONTAINS
      subroutine madronich1_driver(ktau,dtstep,haveaer,              &
...
!
      ...
      real :: xtime,xhour,xmin,gmtp,r_d,uvb_dd1,uvb_du1,uvb_dir1
     ...
     !print *,'gmt,julday in madronich1= ',gmt,julday
      ...
      gmtp=mod(xhour,24.)
      gmtp=gmtp+xmin/60.
     !print *,'gmtp','gmtp = ',gmtp,xhour,xmin
      do 100 j=jts,jte
      do 100 i=its,ite
!     write(0,*)i,j
        do k=kts,kte+1
...
END SUBROUTINE madronich1_driver
      ...
      SUBROUTINE photolysis_mad(mkxcc,a,zmm5,tmm5,pmm5,o3mm5,aerext,wlmm5, &
          dobsnew,phot1,iprt,uvb_dd1,uvb_du1,uvb_dir1)
        ! ----- INPUT ------------------------------------------------
        ! Input from MM5
        ! ------ OUTPUT ----------------------------------------------
        ! -----------------------------------------------------------------------
...
!write(0,*)' here7a zz = zmm5 ',k,zmm5(k)
       ...
       ! znorm = abs((50.-zz(mkxcc+1))/(50.-20.)) !lzhang

        DO k = 1, nabv
          zz(mkxcc+1+k) = 55. - znorm*(55.-zabv(k))
          !zz(mkxcc+1+k) = 51. - znorm*(51.-zabv(k))  !lzhang
!write(6,*)' here8a ',k,' zz(',mkxcc+1+k,') ',zz(mkxcc+1+k),znorm,zabv(k)
        END DO
...
30        IF (zz(k)<=zabv(kk)) THEN
            ...
            cloud(k) = 0.
            ! if(iprt.eq.1)then
            !write(6,'(2i3,5e12.3)')  k,kk,zz(k),zabv(kk),ff,tabv(kk),air(k)
            !print*,  k,kk,zz(k),zabv(kk),ff,tabv(kk),air(k)
            ! endif
          ELSE
40          zu = zabv(kk)
            tu = tabv(kk)
...
!                         ! UV in Photonen/cm**2/s
        ...
        DO nr = 1, nreakj - 1
          ...
          IF (iprt==1 .AND. nr==3) WRITE (06,'(5e12.4)') (hilf1(k),k=1, &
            mkxcc+1)
...
20      CONTINUE
      ...
      SUBROUTINE leqt1b(a,n,nlc,nuc,ia,b,m,ib,ijob,xl)
        ...
        REAL :: a(ia,5), b(ib,m), xl(n,5) !lzhang
        ! .. Local Scalars ..
...
160     CONTINUE
      ...
      SUBROUTINE optics(iprt,a,vair,arayl,gray,omray,ao2,vo3,ao3,vcld,gcld, &
        ...
        ! &nlayer,nlevel,nsurf
         !do kl=1,nlevel
         !write(6,*) 'zl-opt',vair(kl),vo3(kl),vcld(kl),vaer(kl)
         !write(6,*) ao2
         !enddo
        ! endif
        ! stop
        ...
        DO 30 kl = kl0, kl1
          ...
          DO 10 lay = nsurf, nlayer
            ...
            if (dtau(ii)==0.0 .or.dtscat==0.0) then
            endif
...
80      CONTINUE
      ...
      SUBROUTINE calc_zenith(lat,long,ijd,gmt,azimuth,zenith)
        ...
        ! azimuth
        ! .. Scalar Arguments ..
        REAL :: azimuth, gmt, lat, long, zenith,nlong
        INTEGER :: ijd
        ! .. Local Scalars ..
        ...
        lbgmt = 12.0 - eqt/3600. + nlong*24./360.
