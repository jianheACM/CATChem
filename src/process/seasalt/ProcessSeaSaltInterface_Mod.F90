...
!! Generation date: 2025-08-22T23:57:10.678136
...
contains
   ...
   subroutine process_init(this, state_manager, rc)
      ...
      call this%register_process_diagnostics(state_manager, rc)
   ...
   subroutine register_process_diagnostics(this, state_manager, rc)
      use DiagnosticInterface_Mod, only: DiagnosticFieldType, DiagnosticRegistryType, DIAG_REAL_2D, DIAG_REAL_3D
      ...
      call diag_mgr%register_process('seasalt', state_manager, rc)
      if (rc /= CC_SUCCESS) return
      ...
      call diag_field%create('seasalt_mass_emission_total', &
                            'Sea salt mass emission flux total', &
                            'ug/m2/s', DIAG_REAL_2D, 'seasalt', rc)
      if (rc /= CC_SUCCESS) return
      
      ! Initialize data storage for field
      call diag_field%initialize_data(dims_2d, rc)
      if (rc /= CC_SUCCESS) return
      ...
      call diag_field%create('seasalt_number_emission_total', &
                            'Sea salt number emission flux total', &
                            '#/m2/s', DIAG_REAL_2D, 'seasalt', rc)
      if (rc /= CC_SUCCESS) return
      ...
      case ('gong97')
         ...
         call diag_field%create('seasalt_mass_emission_per_bin', &
                               'Sea salt mass emission flux per bin', &
                               'ug/m2/s', DIAG_REAL_3D, 'seasalt', rc)
         if (rc /= CC_SUCCESS) return
         
         ! Initialize data storage for field
         call diag_field%initialize_data(dims_3d_species, rc)
         if (rc /= CC_SUCCESS) return
         
         call registry%register_field(diag_field, rc)
         if (rc /= CC_SUCCESS) return
         
         call diag_field%create('seasalt_number_emission_per_bin', &
                               'Sea salt number emission flux per bin', &
                               '#/m2/s', DIAG_REAL_3D, 'seasalt', rc)
         if (rc /= CC_SUCCESS) return
   ...
   subroutine calculate_and_update_diagnostics(this, column, rc)
      ...
      call this%update_diagnostics('seasalt_mass_emission_total', &
                                       this%column_seasalt_mass_emission_total, &
                                       i_col, j_col, rc)
      if (rc /= CC_SUCCESS) return
      
      ! Scalar diagnostic field
      call this%update_diagnostics('seasalt_number_emission_total', &
                                       this%column_seasalt_number_emission_total, &
                                       i_col, j_col, rc)
      if (rc /= CC_SUCCESS) return
      ...
      case ("gong97")
         ...
         if (allocated(this%column_seasalt_mass_emission_per_bin)) then
            call this%update_diagnostics('seasalt_mass_emission_per_bin', &
                                          this%column_seasalt_mass_emission_per_bin, &
                                          i_col, j_col, rc)
            if (rc /= CC_SUCCESS) return
         end if
         ...
         if (allocated(this%column_seasalt_number_emission_per_bin)) then
            call this%update_diagnostics('seasalt_number_emission_per_bin', &
                                          this%column_seasalt_number_emission_per_bin, &
                                          i_col, j_col, rc)
            if (rc /= CC_SUCCESS) return
         end if
