...
!! Generation date: 2025-11-14T22:58:25.022560
...
contains
   ...
   subroutine run_wesely_scheme_column(this, column, rc)
      ...
      bxheight(1) = met%BXHEIGHT(1)  ! Surface level only
   ...
   subroutine run_gocart_scheme_column(this, column, rc)
      ...
      allocate(species_tendencies(1, n_species))
      ! Allocate meteorological field arrays based on field type and process configuration
      allocate(airden(1))  ! Surface level only
      ...
      allocate(nlevs(1))  ! Surface field - always scalar
      allocate(pblh(1))  ! Surface field - always scalar
      allocate(t(1))  ! Surface level only
      allocate(tstep(1))  ! Special timestep field - scalar
      ...
      allocate(zmid(1))  ! Surface level only
      ...
      ! Extract required fields from met pointer based on field type and processing mode
      airden(1) = met%AIRDEN(1)  ! Surface level only
      ...
      nlevs(1) = met%NLEVS  ! Surface field - scalar access
      pblh(1) = met%PBLH  ! Surface field - scalar access
      t(1) = met%T(1)  ! Surface level only
      tstep(1) = this%get_timestep()  ! Special timestep field - retrieved from ProcessInterface
      ...
      zmid(1) = met%ZMID(1)  ! Surface level only
   ...
   subroutine run_zhang_scheme_column(this, column, rc)
      ...
      allocate(obk(1))  ! Surface field - always scalar
      allocate(ps(1))  ! Surface field - always scalar
      allocate(rh(1))  ! Surface level only
      allocate(ts(1))  ! Surface field - always scalar
      allocate(tstep(1))  ! Special timestep field - scalar
      allocate(u10m(1))  ! Surface field - always scalar
      allocate(ustar(1))  ! Surface field - always scalar
      ...
      obk(1) = met%OBK  ! Surface field - scalar access
      ps(1) = met%PS  ! Surface field - scalar access
      rh(1) = met%RH(1)  ! Surface level only
      ts(1) = met%TS  ! Surface field - scalar access
      tstep(1) = this%get_timestep()  ! Special timestep field - retrieved from ProcessInterface
      u10m(1) = met%U10M  ! Surface field - scalar access
      ustar(1) = met%USTAR  ! Surface field - scalar access
   ...
   function get_required_met_fields(this) result(field_names)
      ...
      process_fields(1) = 'USTAR'
      process_fields(2) = 'TSTEP'
      ...
      select case (trim(this%process_config%drydep_config%gas_scheme))
       case ('wesely')
         ...
         gas_scheme_fields(2) = 'SWGDN'
         gas_scheme_fields(3) = 'SUNCOSmid'
         gas_scheme_fields(4) = 'OBK'
         gas_scheme_fields(5) = 'CLDFRC'
         gas_scheme_fields(6) = 'BXHEIGHT'
         gas_scheme_fields(7) = 'Z0'
         gas_scheme_fields(8) = 'PS'
         gas_scheme_fields(9) = 'FRLAI'
         gas_scheme_fields(10) = 'ILAND'
         gas_scheme_fields(11) = 'SALINITY'
         gas_scheme_fields(12) = 'FRLANDUSE'
         gas_scheme_fields(13) = 'TSKIN'
         gas_scheme_fields(14) = 'LON'
         gas_scheme_fields(15) = 'LAT'
         gas_scheme_fields(16) = 'LUCNAME'
      ...
      select case (trim(this%process_config%drydep_config%aero_scheme))
       case ('gocart')
         ...
         aero_scheme_fields(2) = 'T'
         aero_scheme_fields(3) = 'AIRDEN'
         aero_scheme_fields(4) = 'ZMID'
         aero_scheme_fields(5) = 'LWI'
         aero_scheme_fields(6) = 'PBLH'
         aero_scheme_fields(7) = 'HFLUX'
         aero_scheme_fields(8) = 'Z0H'
       ...
       case ('zhang')
         ...
         aero_scheme_fields(2) = 'OBK'
         aero_scheme_fields(3) = 'BXHEIGHT'
         aero_scheme_fields(4) = 'Z0'
         aero_scheme_fields(5) = 'RH'
         aero_scheme_fields(6) = 'PS'
         aero_scheme_fields(7) = 'U10M'
         aero_scheme_fields(8) = 'V10M'
         aero_scheme_fields(9) = 'FRLANDUSE'
         aero_scheme_fields(10) = 'ILAND'
