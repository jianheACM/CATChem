!> \file dust_example.F90
!! \brief Example usage of dust process
!!
...
program dust_example
   ...
   write(output_unit, '(A)') "5. Dust process created"
   ...
   if (error_handler%has_error()) then
      write(error_unit, '(A)') "Error initializing dust process"
      call error_handler%print_errors()
      stop 1
   end if
   write(output_unit, '(A)') "7. Dust process initialized"
...
contains
   ...
   subroutine setup_meteorological_fields(state_manager, error_handler)
      ...
      call state_manager%add_met_field('solar_zenith_angle', error_handler)
      if (error_handler%has_error()) return
      call state_manager%add_met_field('leaf_area_index', error_handler)
      if (error_handler%has_error()) return
   ...
   subroutine initialize_test_meteorology(state_manager, error_handler)
      ...
      do i_col = 1, n_columns
         do i_lev = 1, n_levels
            ...
            if (error_handler%has_error()) return
            call state_manager%set_met_field('solar_zenith_angle', i_col, i_lev, &
               1.0_fp, error_handler)  ! Default value
            if (error_handler%has_error()) return
            call state_manager%set_met_field('leaf_area_index', i_col, i_lev, &
               1.0_fp, error_handler)  ! Default value
            if (error_handler%has_error()) return
